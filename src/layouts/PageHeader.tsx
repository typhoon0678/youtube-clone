...
export default function PageHeader() {
    ...
    return <div className="flex gap-10 lg:gap-20 justify-between pt-2 mb-6 mx-4">
        <div className={`gap-4 items-center flex-shirink-0 ${showFullWidthSearch ? "hidden" : "flex"
            }`}>
            <Button variant="ghost" size="icon">
                <Menu />
            </Button>
            <a href="/">
                <img src={logo} className='h-6' width="120" />
            </a>
        </div>
        <form className={`gap-4 flex-grow justify-center ${showFullWidthSearch ? "flex" : "hidden md:flex"}`}>
            ...
            <Button type="button" size="icon" className="flex-shirink-0">
                <Mic />
            </Button>
        </form>
        <div className={`flex flex-shirink-0 md:gap-2 ${showFullWidthSearch ? "hidden" : "flex"
            }`}>
